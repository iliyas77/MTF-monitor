...
/**
 * O24 — Non-blocking sync indicator in the app header.
 */
(function (global) {
    'use strict';
    ...
    function syncHosts() {
        return [
            document.getElementById('appHeaderDefaultActions'),
            document.getElementById('appHeaderSettingsSyncHost'),
            document.getElementById('appHeaderSubpageSyncHost')
        ].filter(Boolean);
    }
    ...
    function ensureSyncIndicator() {
        ...
        el.id = 'appLoading';
        el.className = 'app-header-sync d-none';
        el.setAttribute('aria-live', 'polite');
        el.setAttribute('aria-busy', 'false');
        el.innerHTML = '<span class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">Syncing</span></span><span id="appLoadingLabel" class="app-header-sync-label">Syncing…</span>';
        const host = document.getElementById('appHeaderDefaultActions');
        if (host) host.insertBefore(el, host.firstChild);
        return el;
    }

    function placeSyncIndicator() {
        ...
        let host = document.getElementById('appHeaderDefaultActions');
        if (settings && !settings.classList.contains('d-none')) {
            host = document.getElementById('appHeaderSettingsSyncHost') || host;
        } else if (subpage && !subpage.classList.contains('d-none')) {
            host = document.getElementById('appHeaderSubpageSyncHost') || host;
        } else if (def && def.classList.contains('d-none')) {
            host = document.getElementById('appHeaderSettingsSyncHost')
                || document.getElementById('appHeaderSubpageSyncHost')
                || host;
        }
...
(function (global) {
    ...
    function wireDateField(input) {
        ...
        trigger.innerHTML =
            '<i class="far fa-calendar text-primary flex-shrink-0" aria-hidden="true"></i>' +
            '<span class="flex-grow-1 text-truncate" data-date-field-text>Select date</span>' +
            '<i class="fas fa-chevron-down text-muted small flex-shrink-0" aria-hidden="true"></i>';
...
(function (global) {
    ...
    const APP_TAG_CLASSES = {
        ...
        open: 'badge rounded-pill app-tag-open flex-shrink-0',
