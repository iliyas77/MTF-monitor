...
- Cross-feature UI primitives are resolved at call time from `global.MTFComponents` with optional-chaining fallbacks so features stay decoupled.
...
- DOM elements are accessed by fixed IDs (`appSheet`, `appHeader`, `page-<feature>`) and toggled visibility with Bootstrap `d-none` / `d-flex` classes.