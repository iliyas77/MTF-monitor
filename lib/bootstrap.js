/**
 * Bootstrap helpers: Modal/Offcanvas/Toast/loading + thin Bootstrap HTML builders.
 * Not a component system — just shared strings and API wrappers.
 */
/**
 * Bootstrap JS API helpers — Modal, Offcanvas, Toast.
 */
(function (global) {
    'use strict';
    ...
    function getOffcanvas(el) {
        if (!el || !bs()) return null;
        return bs().Offcanvas.getOrCreateInstance(el);
    }
    ...
    function onOffcanvasHidden(el, fn) {
        if (!el || typeof fn !== 'function') return;
        el.addEventListener('hidden.bs.offcanvas', fn);
    }
...
(function (global) {
    ...
    function showToast(msg, type = 'success') {
        ...
        if (showBootstrapToast) {
            showBootstrapToast(msg, type);
            return;
        }
        const container = document.getElementById('toastContainer');
        if (!container) return;
        const div = document.createElement('div');
        div.className = `alert ${TOAST_COLORS[type] || TOAST_COLORS.info} mb-2 text-center`;
        div.innerHTML = msg;
...
(function (global) {
    ...
    function renderFilterChip(label, opts = {}) {
        ...
        const onclickAttr = onclick ? ` onclick="${onclick}"` : '';
        return `<button type="button" class="btn btn-sm btn-outline-secondary${selected ? ' active' : ''}${className ? ` ${className}` : ''}"${rangeAttr}${onclickAttr} aria-pressed="${pressed}">${label}</button>`;
    }

    function renderFilterChipGroup(chips, opts = {}) {
        const {
            groupClass = 'btn-group btn-group flex-wrap',
            ariaLabel = '',
            id = ''
        } = opts;
        const idAttr = id ? ` id="${id}"` : '';
...
(function (global) {
    ...
    function renderRangeQuickButton(label, opts = {}) {
        ...
        const flexClass = flexFill ? ' flex-fill' : '';
        const rangeAttr = dataRange !== '' && dataRange !== undefined ? ` data-range="${dataRange}"` : '';
        const onclickAttr = onclick ? ` onclick="${onclick}"` : '';
    ...
    function renderRangeQuickGroup(buttons, opts = {}) {
        ...
        return `<div class="d-flex flex-wrap gap-2${className ? ` ${className}` : ''}"${idAttr}>${html}</div>`;
    }
...
(function (global) {
    ...
    function renderDetailRow(label, value, extra = '') {
        return `<div class="d-flex justify-content-between gap-3 py-2 border-bottom ${extra}"><span class="small text-muted">${label}</span><span class="small fw-medium text-end">${value}</span></div>`;
    }
